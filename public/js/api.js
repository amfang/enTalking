...
var Api = (function() {
  ...
  var context;
  var stt_model = "en-US_BroadbandModel";
  ...
  return {
    ...
    setRequestPayload: function(newPayloadStr) {
      //alert("setRequestPayload: "+newPayloadStr);
  ...
  //参数无法传递js, 调用函数取的当前设置中英文语音模式
  function returnSTTModel() {
    return stt_model;
  }
  //预处理
  ...
  function sendRequest(text, contextIn) {
    ...
    http.onreadystatechange = function() {
      if (http.readyState === 4 && http.status === 200 && http.responseText) {
         ...
         * - 判断返回结果进行预处理 - call_retrieve_and_rank & intent === "off_topic"
         */
        console.log("postConversationMessage -- http.responseText: "+http.responseText);
        Api.preHandling(JSON.parse(http.responseText));
      }
    };
